...
| 2026-09-17 | `D21` | **`f007` op `0x08` = `test_user_led`, claimed provisionally and pending the app team's ratification** (user's decision, 2026-09-17). Blinks the J1 illuminated button lamp on IO23 — the only LED a customer sees. `test_led` (`0x06`) drives D7, the blue debug LED on IO15, which no customer ever sees, so the two are not interchangeable. **`0x08` is also the natural code for `reboot` (`A4`)** — see `A13`. |
| 2026-09-17 | `D22` | **TEST MODE is a real app state (`APP_STATE_TEST_MODE`), not a flag.** `f007` op `0x09` (param 1/0) enters and leaves it; while in it the board performs **no** vend operation at all — coins start no session, START does nothing, Auto Start does not fire, the inactivity timeout does not run, the relay stays off. **Three independent exits, none removable:** the exit op, **60 s with no command** (user's decision), and a BTN1+BTN2 long press. **RAM-only** — a reboot always comes back in normal operation, because a board that boots into test mode cannot be recovered by power-cycling it. |
| 2026-09-17 | `D23` | **The coin slot is inhibited on entering test mode**, and op `0x0A` (param 1/0) re-opens it for coin-path verification (user's decision, 2026-09-17). A machine out of service should not take money it cannot immediately honour. Enabling restarts the pulse count from zero — a snapshot subtraction, so the **banked money total is never touched**. Consequence, deliberate: coins dropped during a test **are** banked and honoured on exit. Discarding them would break the never-discard invariant. |
| 2026-09-17 | `D24` | **The user-button test needs no command to work.** `PIN_USER_BTN` (START, J1 pin 1 / IO18) is an input — the board cannot press it — so its press counter is live for the whole of test mode, and `f007` op `0x0B` only re-zeroes it for a "press it three times" wizard step. Built that way on purpose: the op code is provisional (`A13`), and a test whose only entry point is an unratified code stops working if that code moves. A refusal costs the reset, not the test. |
| 2026-09-17 | `D25` | **The coin count is already published — no new field was needed.** `f003` Live Counters is Read/**Notify** and `counters_record_coin()` marks it dirty at coin *acceptance*, so a coin pushes exactly one notification (`ble_livecounters.cpp` — "an idle board sends nothing at all, and a coin produces exactly one frame"). The app subscribes and waits; exact pulses = Δamount ÷ (`price_per_credit_cents` ÷ 100), and it already reads that price from `f005`. **The button is the one that genuinely needs an allocation** — `sensors_bitmap` bit 4, built behind `-DENABLE_DIAG_BUTTON_BIT`, ON in the default build since 2026-09-23 (`A14`). |
...
| B1 | ~P3~ | ble | **BUILT + hardware-verified 2026-09-14; ON in the default build 2026-09-23** (flag and `esp32dev-devinfo` env removed — an S1 is always `full`). Machines claimed while configOnly need `A7`'s re-claim path | `A7` (app-side) | `[x]` |
...
| B6 | — | ble | ~~Command `f007`~~ — **built 2026-09-15**: all 7 ops, physical ops gated on `app_state_is_idle()`, `AT+SYNC?`. **2026-09-17: `identify` fixed** (it called the empty `buzzer_beep_enter_config()` stub, so it made no sound and its LED pulse was microseconds wide) and **`0x08 test_user_led` added** for the J1 button lamp (`D21`, ratification `A13`). **Test mode added** (`0x09`–`0x0B`, `D22`–`D24`) with `AT+TEST`. Untested on hardware. `reboot` still needs an op code from the app team | — | `[x]` |
...
| A13 | **P0** | app | **Ratify the `f007` op block `0x08`–`0x0B`**, or hand us different codes. Firmware has claimed all three provisionally and they are in the build: `0x08` `test_user_led` (`D21`), `0x09` `test_mode` (`D22`), `0x0A` `test_coin_slot` (`D23`), `0x0B` `test_button` (`D24`). These are the next sequential codes and so also the obvious choices for `A4`'s `reboot` — and the exposure is no longer cosmetic: a collision on `0x09` would make an app command **take a machine out of service**, not just blink a lamp. Raised to P0 on 2026-09-17 for that reason. Must be settled before any board ships with this build | `A4` | `[ ]` |
| A14 | **P1** | app | **Allocate `SensorBit.button = 4`** in `f006`'s `sensors_bitmap`, so the app can *wait on* a user-button press instead of polling. One bit, backward compatible both ways: an old app build ignores an unknown bit, a new one reading a board that never sets it sees 0. Firmware has it **built and gated** behind `-DENABLE_DIAG_BUTTON_BIT` (ON in `esp32dev` since 2026-09-23 — so if they allocate anything other than 4, firmware must change `DIAG_SENSOR_BIT_BUTTON`, because a bit they reassign renders as whatever they made it mean). **The coin side needs nothing**: `f003` already notifies once per coin (`D25`). A raw *pulse* count still has nowhere to live (a counter cannot be appended to `f006` — the fault list is variable-length and terminal), and `AT+TEST?` is the only place it is readable; decide whether a number is wanted beyond the per-coin notification before we ask for more than the one bit | `A3` | `[ ]` |
